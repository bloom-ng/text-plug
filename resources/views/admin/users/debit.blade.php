<x-admin-dashboard activePage="users" page="Users">
    ...
    <form method="POST" action="/admin/debit-users/{{ $user->id }}"
        class="bg-white rounded-3xl p-8 flex flex-col w-full lg:w-[70%] xl:w-[40%]">
        @csrf
        <h1 class="dm-sans-bold font-bold text-3xl my-3">Debit {{ $user->name }}'s Wallet</h1>
        <hr>
        ...
        <button class="bg-[#DF5C0C] rounded-lg py-4 text-white">Debit</button>
    </form>
